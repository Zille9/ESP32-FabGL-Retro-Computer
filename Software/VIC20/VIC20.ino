...
class Menu : public uiApp {
    ...
    void init() {
      ...
      fileBrowser->changeDirectory(USERDIR);  // set absolute path
      //fileBrowser->changeDirectory( fileBrowser->content().exists(DOWNDIR, false) ? DOWNDIR : USERDIR ); // set relative path
      ...
      radioJNone->onChange = [&]() {
        machine->setJoyEmu(JE_None);
      };
      radioJCurs->onChange = [&]() {
        machine->setJoyEmu(JE_CursorKeys);
      };
      radioJMous->onChange = [&]() {
        machine->setJoyEmu(JE_Mouse);
      };
      ...
      // "Download From" label
      new uiStaticLabel(rootWindow(), "Basic32+", Point(5, 326), true, STYLE_LABELGROUP);
...
void basicloader(void) {
  ...
  size_t updateSize = updateBin.size();
    ...
    Serial.println("Could not load Binary from sd");
  }
}
...
void performUpdate(Stream &updateSource, size_t updateSize) {
  ...
  if (Update.begin(updateSize, U_FLASH, 2, 1, " ")) {
    ...
    if (written == updateSize) {
      ...
      Serial.println("Written only : ");
    }
    if (Update.end()) {
      Serial.println("OTA done!");
      if (Update.isFinished()) {
        Serial.println("successfully completed. Now Rebooting.");
        delay(1000);
        ESP.restart();
      }
      else {
        Serial.println("not finished? Something went wrong!");
      }
    }
    else {
