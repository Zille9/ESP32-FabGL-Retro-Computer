    ...
    if (gameState_ == GAMESTATE_PLAYING || gameState_ == GAMESTATE_PLAYERKILLED) {
      ...
      if (IntroScene::controller_ == 1) {
        ...
        if (keyboard->isVKDown(fabgl::VK_SPACE) && !playerFire_->visible)  // player fire?
          fire();
        if (keyboard->isVKDown(fabgl::VK_ESCAPE))     //########### Aenderung fuer Basic32+ #################
        {
          load_basic  = 1; // zum Basic zurückkehren
          stop();
...
void setup()
{
  ...
  //DisplayController.moveScreen(25, -10);  //Korrektur für Pollin-Display
}
...
void load_binary(void) {
  ...
  if ( !SD.begin( kSD_CS, spiSD )) {                        //SD-Card starten
    // mount-fehler
    spiSD.end();                                            //unmount
    canvas.drawText(10, 200, "Mount Error");
    return;
  }
  ...
  if (updateBin) {
    ...
    if (updateSize > 0) {
      ...
      canvas.drawText(10, 200, "Error, file is empty");
    }
    updateBin.close();
  }
  else {
    //canvas.drawText(10, 140, "Could not load Binary from sd");
  }
}
...
void performUpdate(Stream &updateSource, size_t updateSize) {
  if (Update.begin(updateSize, U_FLASH, 2, 1, " ")) {
    ...
    if (written == updateSize) {
      ...
      canvas.drawText(10, 200, "Written error: " );
    }
    if (Update.end()) {
      canvas.drawText(10, 200, "OTA done!");
      if (Update.isFinished()) {
        canvas.drawText(10, 200, "successfully completed. Now Rebooting.");
        delay(1000);
        ESP.restart();
      }
      else {
        canvas.drawText(10, 200, "not finished? Something went wrong!");
      }
    }
    else {
      canvas.drawText(10, 200, "Error Occurred. Error #: ");
    }

  }
  else
  {
    canvas.drawText(10, 200, "Not enough space to begin OTA");
  }
}
