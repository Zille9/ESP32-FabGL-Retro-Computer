...
File32 updateFile = SD.open("/BASIC.BIN", O_RDONLY);
    if (!updateFile) {
    ...
    if (updateSize > 0) {
        Terminal.printf("Update-Datei gefunden (%d Bytes). Starte OTA...\n", updateSize);
        
        // Start des ESP32-internen Flash-Prozesses
        if (Update.begin(updateSize, U_FLASH)) {
            
            // FEHLERBEHEBUNG: Array-Puffer korrekt deklariert (uint8_t buffer[256])
            uint8_t buffer[256];
            size_t written = 0;
            ...
            if (written == updateSize) {
                Terminal.println("Firmware erfolgreich in Flash geschrieben!");
            } else {
                Terminal.printf("Fehler: Nur %d/%d Bytes geschrieben\n", written, updateSize);
            }

            if (Update.end()) {
                if (Update.isFinished()) {
                    Terminal.println("SUCCESS: Loesche Update-Datei und starte neu...");
                    updateFile.close();
                    
                    delay(500);
                    ESP.restart();
                } else {
                    Terminal.println("Fehler: Update beendet, aber nicht abgeschlossen.");
                }
            } else {
                Terminal.printf("Fehler bei Update.end(): %s\n", Update.errorString());
            }
        } else {
            Terminal.println("Fehler: Nicht genug Platz in der OTA-Partition.");
        }
    }
    updateFile.close();
}
